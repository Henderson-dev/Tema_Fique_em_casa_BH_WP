...
Créditos
==================================================================
Template desenvolvido pela Equipe Calebe_
Direção geral: Calebe Bezerra
Conteúdo: Érika Gimenes
Design: Bernardo Costa
Coordenação de equipe: Giuliano Silva
Atendimento: Iza Castro
Coordenação de Equipe de Programação / Back-end: Henderson Torres
Programação / Back-end: Leandro Hindu
Programação / Front-end: Mateus Carvalho
==================================================================
Acesse: calebedesign.com.br 

Projeto não comercial sem fins lucrativos.
Atribuição-NãoComercial-CompartilhaIgual
CC BY-NC-SA

Esta licença permite que outros remixem, adaptem e criem a partir do seu trabalho 
para fins não comerciais, desde que atribuam a você o devido crédito e que 
...
/*if ( ! function_exists('wpsf_maintenance_mode') ) :
	...
	function wpsf_maintenance_mode() { 
		...
		if ( !is_user_logged_in() ) { 
			echo '<h1>Site em manutenção</h1>'; // Coloque aqui o HTML que será exibido na página
...
if( ! function_exists( 'wpsf_login_header_title' ) ) : 
	...
	add_filter( 'login_headertitle', 'wpsf_login_header_title' );
		...
		*/
		echo '<style type="text/css">
				body.login div#login h1 a {
					background-image: url(' . TEMPLATE_STYLE . '/img/logo.png);
...
if ( ! function_exists( 'wpsf_custom_admin_footer' ) ) :
	...
	function wpsf_custom_admin_footer() {
		...
		echo 'Criado com <a href="http://www.wordpress.org" target="_blank">WordPress</a> | Desenvolvido por <a href="http://www.calebedesign.com.br" target="_blank">Calebe_</a>';
	}
...
//add_image_size( 'tamanho-exemplo', 300, 450 );
...
if ( ! function_exists( 'wpsf_enqueue_assets' ) ) :
	...
	function wpsf_enqueue_assets() {
		...
		// Chave API do Maps
		$chave_maps = get_field('chave_maps','option');
		//Inclui os JS
		//wp_enqueue_script('maps_api', '//maps.googleapis.com/maps/api/js?sensor=false&key=AIzaSyA8oVdLcmU3bmfGfYeI_PAFwZo7tsfI_Vg', '', '', true);
		wp_enqueue_script('maps_api', '//maps.googleapis.com/maps/api/js?sensor=false&key='.$chave_maps, '', '', true);
		
		wp_enqueue_script('mask', 'https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.14.16/jquery.mask.min.js?ver=1.0', '', '', true);

		wp_enqueue_script('bootstrap', TEMPLATE . '/vendor/bootstrap/js/bootstrap.bundle.min.js', array('jquery'), '', true);
			
		//Inclui os CSS 
		wp_enqueue_style( 'fontawesome', TEMPLATE . '/vendor/fontawesome-free/css/all.min.css' );
		wp_enqueue_style( 'simple', TEMPLATE . '/vendor/simple-line-icons/css/simple-line-icons.css' );
		wp_enqueue_style( 'landing', TEMPLATE . '/css/landing-page.min.css' );	
...
/**
 *	PEGA A LATITUDE E LONGITUDE QUANDO CADASTRAR O LOCAL
 */
if ( ! function_exists('pega_lat_long')) :

	function pega_lat_long( $post_id, $post, $update ){
		
		if ( wp_is_post_revision($post_id)) return;
		if ( $post->post_type != 'post' ) return;
    	
    	if( get_field('rua', $post_id) == '' or get_field('bairro', $post_id) == '' or get_field('cidade', $post_id) == '') return;

		$endereco = get_field('rua', $post_id) . ', ' . get_field('bairro', $post_id) . ' ' . get_field('cidade', $post_id) . ' MG';
		$endereco = str_replace(' ', '+', $endereco);
		
    // Define a Key de acesso ao Google Maps
    $keycode = "";

		// URL que retorna a lagitude e logintude
		$url = "https://maps.google.com/maps/api/geocode/json?key=". $keycode . "&address=" . $endereco;
		...
		update_field('longitude', $long, $post_id);
		
	}
	
	add_action( 'save_post', 'pega_lat_long', 10, 3 );
